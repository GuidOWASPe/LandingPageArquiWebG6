...
def detectar_forma():
    data = request.json
    imagen_url = data.get("imagen_path")
    
    imagen_path = descargar_imagen(imagen_url)
    if imagen_path is None:
        return jsonify({"nombreForma": "Error", "descripcionForma": "No se pudo descargar la imagen."})
    ...
    with mp_face_mesh.FaceMesh(max_num_faces=1, refine_landmarks=True) as face_mesh:
        ...
        if results.multi_face_landmarks:
            for face_landmarks in results.multi_face_landmarks:
                ...
                nombre_forma, descripcion_forma = identificar_forma_rostro(aspecto)
                return jsonify({"nombreForma": nombre_forma, "descripcionForma": descripcion_forma})

    return jsonify({"nombreForma": "Error", "descripcionForma": "No se detectaron rostros en la imagen."})
